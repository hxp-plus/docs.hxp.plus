...
其中 `/etc/conf.d/display-manager` 已正确配置 `DISPLAYMANAGER="sddm"` ，且使用配置 `~/.xinitrc` 后用 `startx` 命令启动 KDE ，或者使用 VNC 都正常。
...
!!! tip
    ...
    如果后续 Gentoo 内核升级，建议重新检查 `hyperv_fb` 和 `hyperv_drm` 的配置状态，避免图形问题复发。
