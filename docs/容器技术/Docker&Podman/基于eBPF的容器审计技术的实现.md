...
某项目需要监视运行在 podman 宿主机上所有容器中的文件修改和系统调用，作为审计日志。日志中至少需要包含执行命令的容器、命令内容或修改的文件。

使用 auditd 无法实现此需求，因为容器内 auditd 无法启动，audit 依赖于内核 kaudit 进程，其工作方式为内核态 kauditd 在系统调用层记录日志，把日志放入一个 netlink 套接字中，用户态 auditd 进程监听这个套接字，将日志转储到系统日志或日志文件中。且一台机器上只要有一个容器或宿主机开启了 auditd，其他容器或宿主机就无法再次启动 auditd。

而且，auditd 不支持 namespace，无法确定是哪个容器执行了什么命令。因此，解决这一问题需要使用 eBPF 内核探针。
