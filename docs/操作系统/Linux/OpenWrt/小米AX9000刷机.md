...
!!! warning
    ...
    刷机有风险，操作前请确保已备份重要数据，并了解救砖方法。

## 确保小米固件在分区 rootfs 里

SSH 登录小米路由器，设置 env 保证小米固件在分区 rootfs 里，为下一步把 qsdk 刷到 rootfs_1 做准备：
...
刷机固件为[kiddin9 固件](./attachments/AX9000固件_20240929.7z)， scp 把固件 kwrt-09.29.2024-ipq807x-generic-xiaomi_ax9000-squashfs-factory.ubi 传到路由器 tmp 目录，ssh 登录后执行以下命令：
