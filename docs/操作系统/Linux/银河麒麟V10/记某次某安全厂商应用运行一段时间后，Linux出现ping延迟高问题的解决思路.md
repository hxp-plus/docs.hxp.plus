...
在某台联想信创服务器（海光 CPU 版本）上安装银河麒麟 V10SP1 后，安装了某个安全厂商的软件。该安全厂商软件需要部分 root 权限运行，观察到的现象为：软件运行一段时间后，该节点到其他节点的 ping 延迟变得特别高且不稳定（同一交换机下的两台机器互相 ping，延迟高达 100ms）。且观察到 CPU 软中断非常高且分布不均匀。
...
- 关闭该物理机上安装的安全防护软件 G01，并卸载其内核模块，问题依旧存在，说明不是 G01 导致的问题。
